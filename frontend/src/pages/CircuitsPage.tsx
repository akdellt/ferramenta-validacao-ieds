...
function CircuitsPage() {
  ...
  if (!relatorioResultados) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-4">
        ...
        <button onClick={() => navigate("/topologia")} className="text-eq-primary font-bold hover:underline">
  ...
  const { is_valid, summary, ied_summary, filename, scenario } = relatorioResultados;
                ...
                {ied.errors.length > 0 && (
                  <div className="p-5 bg-white">
                    <ul className="space-y-3">
                      {ied.errors.map((error: string, idx: number) => (
...
function StatCard({ title, count, type }: { title: string, count: number, type: 'danger' | 'warning' | 'info' | 'neutral' }) {
  const styles: any = {
    danger: "text-rose-600 border-rose-100 bg-white",
    warning: "text-orange-600 border-orange-100 bg-white",
    info: "text-blue-600 border-blue-100 bg-white",
    neutral: "text-gray-800 border-gray-200 bg-white"
  };

  return (
    <div className={`p-6 rounded-3xl border-2 shadow-sm flex flex-col items-center transition-transform hover:-translate-y-1 ${styles[type]}`}>
      <span className="text-4xl font-black leading-none mb-2">{count}</span>
      <span className="text-[10px] font-black uppercase tracking-wider text-gray-400">{title}</span>
    </div>
  );
}
